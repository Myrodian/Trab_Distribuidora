...
-- -----------------------------------------------------
-- Table `Distribuidora`.`Pais`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Distribuidora`.`Pais` ;

CREATE TABLE IF NOT EXISTS `Distribuidora`.`Pais` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `Nome` VARCHAR(100) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) )
...
CREATE TABLE IF NOT EXISTS `Distribuidora`.`Estado` (
  ...
  CONSTRAINT `fk_Estado_Pais`
    FOREIGN KEY (`Pais_id`)
    REFERENCES `Distribuidora`.`Pais` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Distribuidora`.`Entrega` (
  ...
  `destino_tipo` ENUM('cliente final', 'filial') NOT NULL,
  `observacoes` TEXT NULL,
  `Funcionario_id` INT UNSIGNED NOT NULL,
  `Rua_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Entrega_Rua1_idx` (`Rua_id` ASC) ,
  INDEX `fk_Entrega_Funcionario1_idx` (`Funcionario_id` ASC) ,
  CONSTRAINT `fk_Entrega_Rua1`
    FOREIGN KEY (`Rua_id`)
